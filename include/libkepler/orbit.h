...
static inline void orbit_from_state(
    ...
    if(dot(h, h)[0] < DBL_EPSILON) {
        // radial trajectory
        if(dot(pos, pos)[0] < DBL_EPSILON) {
            // zero trajectory
            orbit->gravity_parameter = mu;
            orbit->orbital_energy = -INFINITY;
            orbit->angular_momentum = 0.0;
            orbit->periapsis_time = epoch;
            orbit->major_axis = (vec4d){ 1.0, 0.0, 0.0, 0.0 };
            orbit->minor_axis = (vec4d){ 0.0, 1.0, 0.0, 0.0 };
            orbit->normal_axis = (vec4d){ 0.0, 0.0, 1.0, 0.0 };
            return;
        }

        vec4d major = dot(pos, pos)[0] < DBL_EPSILON ?
            unit4d(vel) : unit4d(pos);
