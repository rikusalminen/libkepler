...
#include <libkepler/orientation.h>

#include "numtest.h"

void orbit_from_state_test_new(
    double *params,
    int num_params,
    void *extra_args,
    ...
    if(conic_circular(e)) { // "fix" circular orbits
        ...
        if(zero(dot(nodes, nodes)[0])) { // circular and equatorial
        ...
        f = -sign(dot(major_axis, horizontal)[0]) *
            acos(clamp(-1.0, 1.0, dot(major_axis, radial)[0]));
    }
    ...
    vec4d pos = splat4d(r) * radial + (vec4d){ 0.0, 0.0, 0.0, 1.0 };
    ...
    if(!conic_circular(e)) {
        vec4d posw = xyz4d(pos);
        double v2 = dot(vel, vel)[0];
        vec4d ecc = splat4d(1.0/mu) * (splat4d(v2 - mu/r)*posw - dot(posw, vel) * vel);

        ASSERT_EQF(dot(ecc, orbit.major_axis)[0], e,
            "Eccentricity vector");
    }
...
void orbit_from_elements_test_new(
    double *params,
    int num_params,
    void *extra_args,
    struct numtest_ctx *test_ctx) {
