...
void intercept_simple_test(double *params, int num_params, void *extra_args, struct numtest_ctx *test_ctx) {
    ...
    struct kepler_elements orbit2 = { 1.0, 0.0, 1.0, 5*M_PI/180.0, 0.0, 0.0, 0.0 };
...
void intercept_test(double *params, int num_params, void *extra_args, struct numtest_ctx *test_ctx) {
    ...
    double P1 = kepler_orbit_closed(&orbit1) ?
        kepler_orbit_period(&orbit1) : (M_PI/2 / kepler_orbit_mean_motion(&orbit1));
    double P2 = kepler_orbit_closed(&orbit2) ?
        kepler_orbit_period(&orbit2) : (M_PI/2 / kepler_orbit_mean_motion(&orbit2));

    double dt = P1 + P2;
    bool hit = intercept_orbit(&orbit1, &orbit2, t0-dt, t0+dt, NULL); // XXX: null
    ASSERT(hit, "Intercept time interval found");
}
