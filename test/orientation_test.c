...
void orientation_test(
    ...
    ASSERT_EQF(mag(major)[0], 1.0,
        "Major axis is an unit vector");
    ASSERT_EQF(mag(minor)[0], 1.0,
        "Minor axis is an unit vector");
    ASSERT_EQF(mag(normal)[0], 1.0,
        "Normal axis is an unit vector");

    ASSERT(ZEROF(major[3]) && ZEROF(minor[3]) && ZEROF(normal[3]),
        "w component is zero");

    ASSERT(ZEROF(dot(major, minor)[0]) &&
        ZEROF(dot(major, normal)[0]) &&
        ZEROF(dot(minor, normal)[0]),
        "Axes are orthogonal");
