#include <libkepler/stumpff.h>

#include <math.h>

#include "numtest.h"

void stumpff_test(double *params, int num_params, void *extra_args, struct numtest_ctx *test_ctx) {
    (void)extra_args;
    ASSERT(num_params == 2, "");
    ...
    for(int i = 0; i < 4; ++i)
        ASSERT(isfinite(cs_fast[i]), "Stumpff fast series c%d not NaN", i);

    for(int i = 0; i < 2; ++i)
        ASSERT_EQF(1.0 - cs_fast[i], z*cs_fast[i+2], "Stumpff fast series c%d recurrence relation", i);

    for(int i = 0; i < 4; ++i)
        ASSERT_EQF(cs[i], cs_fast[i], "Stumpff fast series and series c%d are equal", i);

    double cs_four[4];
    stumpff_fast(4.0 * z, cs_four);

    // quadruple angle formulae
    ASSERT_EQF(cs_four[0], 2.0*cs[0]*cs[0] - 1.0, "c_0(4z) = 2*c_0(z)^2 - 1");
    ASSERT_EQF(cs_four[1], cs[0]*cs[1], "c_1(4z) = c_0(z)*c_1(z)");
    ASSERT_EQF(cs_four[2], cs[1]*cs[1] / 2.0, "c_2(4z) = c_1(z)^2 / 2");
    ASSERT_EQF(cs_four[3], (cs[2] + cs[0]*cs[3])/4.0, "c_3(4z) = (c_2(z) + c_0(z)*c_3(z))/4");
}
